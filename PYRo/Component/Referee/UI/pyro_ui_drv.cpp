 ...
 * @brief RoboMaster referee client UI helper implementation.
 */
                               ...
                               uint16_t start_y, float value)
{
    ...
    const int32_t i_val = static_cast<int32_t>(std::round(value * 1000.0f));
    uint32_t u_val      = 0;
    std::memcpy(&u_val, &i_val, sizeof(u_val));
...
ui_drv_t &ui_drv_t::draw_int(const char name[3], ui_operate op, uint8_t layer,
                             ui_color color, uint16_t font_size,
                             uint16_t width, uint16_t start_x,
                             uint16_t start_y, int32_t value)
{
    auto fig      = create_base_figure(name, op, ui_figure::INT, layer, color,
                                       width, start_x, start_y);
    fig.details_a = font_size;

    const auto u_val = static_cast<uint32_t>(value);
    fig.details_c    = (u_val & 0x3FF);
    fig.details_d    = ((u_val >> 10) & 0x7FF);
    fig.details_e    = ((u_val >> 21) & 0x7FF);
                           ...
                           const std::string &text) const
{
    ...
    if (len > sizeof(str_pkt.text))
    {
        len = sizeof(str_pkt.text);
    }
    str_pkt.figure.details_b = len;

    std::memset(str_pkt.text, 0, sizeof(str_pkt.text));
    std::memcpy(str_pkt.text, text.c_str(), len);

    return _referee->send_ui_interaction(
        static_cast<uint16_t>(interaction_sub_cmd::UI_CMD_DRAW_CHAR),
        &str_pkt);
}
...
bool ui_drv_t::flush()
{
    ...
    {
        const size_t remain = _buffer.size() - i;
