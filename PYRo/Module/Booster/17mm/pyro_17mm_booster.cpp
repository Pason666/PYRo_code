...
// 速度与角度设定
constexpr float SHOOT_BULLET_MUZZLE_VELOCITY = 28.5f; // 枪口初速度
...
constexpr float TRIGGER_CONTINUOUS_RADPS = -15.0f; // 连续发射速度（拨弹盘速度）
...
void shoot_17mm_control_t::state_ready_shoot_t::execute(owner *ctx)
{
    ...
    if (ctx->_ctx.cmd->single_shoot)
    {
        ...
        {
            this->request_switch(&ctx->_state_reverse);
        }
        else
        {
...
{
    if (std::abs(ctx->_ctx.data.current_trig_radps - TRIGGER_CONTINUOUS_RADPS) >
        TRIGGER_BLOCK_RADPS)
    {
...
void shoot_17mm_control_t::state_single_bullet_t::execute(owner *ctx)
{
    ...
    float err = std::abs(ctx->_ctx.data.current_trig_rad -
                         ctx->_ctx.data.target_trig_rad);
    if (err > TRIGGER_BLOCK_RAD)
    {
        ctx->_ctx.data.block_time++;
        if (ctx->_ctx.data.block_time > TRIGGER_BLOCK_TIME)
        {
            this->request_switch(&ctx->_state_adjust);
        }
    }
    else
    {
