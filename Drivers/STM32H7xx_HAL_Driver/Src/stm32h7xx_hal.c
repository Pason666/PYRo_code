...
#define __STM32H7xx_HAL_VERSION                                                \
    ((__STM32H7xx_HAL_VERSION_MAIN << 24) |                                    \
     ...
     (__STM32H7xx_HAL_VERSION_SUB2 << 8) | (__STM32H7xx_HAL_VERSION_RC))
 ...
 * from the internal HSI at 16 MHz). Set NVIC Group Priority to 4. Calls the
 * HAL_MspInit() callback function defined in user file "stm32h7xx_hal_msp.c" to
...
HAL_StatusTypeDef HAL_Init(void)
{
    ...
    __HAL_ART_CONFIG_BASE_ADDRESS(
        0x08100000UL);  /* Configure the Cortex-M4 ART Base address to the Flash
                           Bank 2 : */
...
void HAL_SYSCFG_ETHInterfaceSelect(uint32_t SYSCFG_ETHInterface)
{
    ...
    MODIFY_REG(SYSCFG->PMCR, SYSCFG_PMCR_EPIS_SEL,
 ...
 * (PXn and PXn_C) This parameter can be one or a combination of the following
 * values:
 ...
 * @param  BootRegister :Specifies the Boot Address register (Address0 or
 * Address1) This parameter can be one of the following values:
...
void HAL_SYSCFG_CM7BootAddConfig(uint32_t BootRegister, uint32_t BootAddress)
{
    ...
    if (BootRegister == SYSCFG_BOOT_ADDR0)
    {
        ...
        MODIFY_REG(SYSCFG->UR2, SYSCFG_UR2_BCM7_ADD0,
                   ((BootAddress >> 16) << SYSCFG_UR2_BCM7_ADD0_Pos));
#else
...
void HAL_SYSCFG_CM4BootAddConfig(uint32_t BootRegister, uint32_t BootAddress)
{
    ...
    if (BootRegister == SYSCFG_BOOT_ADDR0)
    {
        ...
        MODIFY_REG(SYSCFG->UR3, SYSCFG_UR3_BCM4_ADD0,
                   ((BootAddress >> 16) << SYSCFG_UR3_BCM4_ADD0_Pos));
    }
...
#else
    SET_BIT(SYSCFG->CCCSR, (SYSCFG_CCCSR_HSLV0 | SYSCFG_CCCSR_HSLV1 |
                            SYSCFG_CCCSR_HSLV2 | SYSCFG_CCCSR_HSLV3));
...
#else
    CLEAR_BIT(SYSCFG->CCCSR, (SYSCFG_CCCSR_HSLV0 | SYSCFG_CCCSR_HSLV1 |
                              SYSCFG_CCCSR_HSLV2 | SYSCFG_CCCSR_HSLV3));
#endif /* SYSCFG_CCCSR_HSLV */
}
 ...
 * SYSCFG_CCVR)
 *   @arg SYSCFG_REGISTER_CODE: Select Code from the SYSCFG compensation cell
 * code register (SYSCFG_CCCR)
 * @retval None
 */
                                       ...
                                       uint32_t SYSCFG_NMOSCode)
{
    ...
    MODIFY_REG(
        SYSCFG->CCCR, SYSCFG_CCCR_NCC | SYSCFG_CCCR_PCC,
                                              ...
                                              uint32_t SYSCFG_NMOSCode)
{
    ...
    assert_param(IS_SYSCFG_CODE_CONFIG(SYSCFG_NMOSCode));
    MODIFY_REG(
        SYSCFG->CCCR, (SYSCFG_CCCR_NCC_MMC | SYSCFG_CCCR_PCC_MMC),
        (((uint32_t)(SYSCFG_PMOSCode) << 4) | (uint32_t)(SYSCFG_NMOSCode)));
}
...
/**
  ...
  * @param  BankMapConfig: Defines the FMC Bank mapping configuration. This
  parameter can be FMC_SWAPBMAP_DISABLE, FMC_SWAPBMAP_SDRAM_SRAM,
 ...
 * @note    No edge configuration for direct lines but for configurable
 * lines:(EXTI_LINE0..EXTI_LINE21),
 *          EXTI_LINE49,EXTI_LINE51,EXTI_LINE82,EXTI_LINE84,EXTI_LINE85 and
 * EXTI_LINE86.
 * @param   EXTI_Line: Specifies the EXTI LINE, it can be one of the following
 * values, (EXTI_LINE0....EXTI_LINE87)excluding :line45, line81,line83 which are
 * reserved
 * @param   EXTI_Edge: Specifies  EXTI line Edge used.
 *          This parameter can be one of the following values :
 *   @arg EXTI_RISING_EDGE : Configurable line, with Rising edge trigger
 * detection
 *   @arg EXTI_FALLING_EDGE: Configurable line, with Falling edge trigger
 * detection
 * @retval None
 */
void HAL_EXTI_EdgeConfig(uint32_t EXTI_Line, uint32_t EXTI_Edge)
{
    ...
    CLEAR_BIT(*(__IO uint32_t *)(((uint32_t)&(EXTI->FTSR1)) +
                                 ((EXTI_Line >> 5) * 0x20UL)),
              (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));
    CLEAR_BIT(*(__IO uint32_t *)(((uint32_t)&(EXTI->RTSR1)) +
                                 ((EXTI_Line >> 5) * 0x20UL)),
              (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));

    if ((EXTI_Edge & EXTI_RISING_EDGE) == EXTI_RISING_EDGE)
    {
        SET_BIT(*(__IO uint32_t *)(((uint32_t)&(EXTI->RTSR1)) +
                                   ((EXTI_Line >> 5) * 0x20UL)),
                (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));
    }
    if ((EXTI_Edge & EXTI_FALLING_EDGE) == EXTI_FALLING_EDGE)
    {
        SET_BIT(*(__IO uint32_t *)(((uint32_t)&(EXTI->FTSR1)) +
                                   ((EXTI_Line >> 5) * 0x20UL)),
                (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));
    }
}
 ...
 * @param   EXTI_Line: Specifies the EXTI LINE, it can be one of the following
 * values,
 *          (EXTI_LINE0..EXTI_LINE21),EXTI_LINE49,EXTI_LINE51,EXTI_LINE82,EXTI_LINE84,EXTI_LINE85
 * and EXTI_LINE86.
 * @retval None
 */
void HAL_EXTI_GenerateSWInterrupt(uint32_t EXTI_Line)
{
    ...
    SET_BIT(*(__IO uint32_t *)(((uint32_t)&(EXTI->SWIER1)) +
                               ((EXTI_Line >> 5) * 0x20UL)),
            (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));
}
...
void HAL_EXTI_D1_ClearFlag(uint32_t EXTI_Line)
{
    ...
    WRITE_REG(*(__IO uint32_t *)(((uint32_t)&(EXTI_D1->PR1)) +
                                 ((EXTI_Line >> 5) * 0x10UL)),
              (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));
}
...
void HAL_EXTI_D2_ClearFlag(uint32_t EXTI_Line)
{
    ...
    WRITE_REG(*(__IO uint32_t *)(((uint32_t)&(EXTI_D2->PR1)) +
                                 ((EXTI_Line >> 5) * 0x10UL)),
              (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));
}
...
/**
  ...
  * @param   EXTI_Line: Specifies the EXTI LINE, it can be one of the following
  values,
  *         (EXTI_LINE0....EXTI_LINE87)excluding :line45, line81,line83 which
  are reserved
  * @param   EXTI_Mode: Specifies which EXTI line is used as interrupt or an
  event.
  *          This parameter can be one or a combination of the following values
  :
                                  ...
                                  uint32_t EXTI_LineCmd)
{
    ...
    if ((EXTI_Mode & EXTI_MODE_IT) == EXTI_MODE_IT)
    {
        if (EXTI_LineCmd == 0UL)
        {
            ...
            CLEAR_BIT(*(__IO uint32_t *)(((uint32_t)&(EXTI_D1->IMR1)) +
                                         ((EXTI_Line >> 5) * 0x10UL)),
                      (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));
        }
        else
        {
            SET_BIT(*(__IO uint32_t *)(((uint32_t)&(EXTI_D1->IMR1)) +
                                       ((EXTI_Line >> 5) * 0x10UL)),
                    (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));
        }
    }
    ...
    if ((EXTI_Mode & EXTI_MODE_EVT) == EXTI_MODE_EVT)
    {
        ...
        {
            /* Clear EXTI line configuration */
            CLEAR_BIT(*(__IO uint32_t *)(((uint32_t)&(EXTI_D1->EMR1)) +
                                         ((EXTI_Line >> 5) * 0x10UL)),
                      (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));
        }
        else
        {
            SET_BIT(*(__IO uint32_t *)(((uint32_t)&(EXTI_D1->EMR1)) +
                                       ((EXTI_Line >> 5) * 0x10UL)),
                    (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));
        }
    }
                                  ...
                                  uint32_t EXTI_LineCmd)
{
    ...
    if ((EXTI_Mode & EXTI_MODE_IT) == EXTI_MODE_IT)
    {
        ...
        {
            /* Clear EXTI line configuration */
            CLEAR_BIT(*(__IO uint32_t *)(((uint32_t)&(EXTI_D2->IMR1)) +
                                         ((EXTI_Line >> 5) * 0x10UL)),
                      (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));
        }
        else
        {
            SET_BIT(*(__IO uint32_t *)(((uint32_t)&(EXTI_D2->IMR1)) +
                                       ((EXTI_Line >> 5) * 0x10UL)),
                    (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));
        }
    }
    ...
    if ((EXTI_Mode & EXTI_MODE_EVT) == EXTI_MODE_EVT)
    {
        ...
        {
            /* Clear EXTI line configuration */
            CLEAR_BIT(*(__IO uint32_t *)(((uint32_t)&(EXTI_D2->EMR1)) +
                                         ((EXTI_Line >> 5) * 0x10UL)),
                      (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));
        }
        else
        {
            SET_BIT(*(__IO uint32_t *)(((uint32_t)&(EXTI_D2->EMR1)) +
                                       ((EXTI_Line >> 5) * 0x10UL)),
                    (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));
        }
    }
 ...
 * @param   EXTI_Line: Specifies the EXTI LINE, it can be one of the following
 * values, (EXTI_LINE0...EXTI_LINE15),(EXTI_LINE19...EXTI_LINE21),EXTI_LINE25,
 * EXTI_LINE34, EXTI_LINE35,EXTI_LINE41,(EXTI_LINE48...EXTI_LINE53)
 * @param   EXTI_LineCmd controls (Enable/Disable) the EXTI line.
                                  ...
                                  uint32_t EXTI_ClearSrc)
{
    ...
    {
        /* Clear EXTI line configuration */
        CLEAR_BIT(*(__IO uint32_t *)(((uint32_t)&(EXTI->D3PMR1)) +
                                     ((EXTI_Line >> 5) * 0x20UL)),
                  (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));
    }
    else
    {
        SET_BIT(*(__IO uint32_t *)(((uint32_t)&(EXTI->D3PMR1)) +
                                   ((EXTI_Line >> 5) * 0x20UL)),
                (uint32_t)(1UL << (EXTI_Line & 0x1FUL)));
    }
    ...
    {
        pRegv = (__IO uint32_t *)(((uint32_t)&(EXTI->D3PCR1L)) +
                                  ((EXTI_Line >> 5) * 0x20UL));
    }
    else
    {
        pRegv = (__IO uint32_t *)(((uint32_t)&(EXTI->D3PCR1H)) +
                                  ((EXTI_Line >> 5) * 0x20UL));
    }
    MODIFY_REG(*pRegv, (uint32_t)(3UL << ((EXTI_Line * 2UL) & 0x1FUL)),
               (uint32_t)(EXTI_ClearSrc << ((EXTI_Line * 2UL) & 0x1FUL)));
}
